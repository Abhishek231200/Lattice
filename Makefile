...
demo-wait:
	...
	@for i in $$(seq 1 30); do \
	    if curl -sf http://localhost:5000/health >/dev/null 2>&1 && \
	       curl -sf http://localhost:5002/health >/dev/null 2>&1; then \
...
demo-run:
	...
	$(eval TENANT := $(shell curl -sf -X POST http://localhost:5002/tenants \
	...
	$(eval ROOT_TL := $(shell curl -sf -X POST http://localhost:5000/timelines \
	    -H "Content-Type: application/json" \
	...
	@curl -sf -X POST http://localhost:5002/branches \
	    -H "Content-Type: application/json" \
	...
	@curl -sf http://localhost:5002/autoscaler/decisions | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'  {len(d)} decisions recorded')"

	@echo ""
	@echo "Demo complete. Services:"
	@echo "  Pageserver:    http://localhost:5000"
	@echo "  Control Plane: http://localhost:5002"
